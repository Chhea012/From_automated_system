...
def create_contract_tab():
    with st.form("contract_form", clear_on_submit=True):
        ...
        with col1:
            project_title = st.text_input("Project Title", placeholder="e.g., Consultancy for Budget Analysis")
            contract_number = st.text_input("Contract Number", placeholder="e.g., NGOF/2025-100")
            organization_name = st.text_input("Organization Name", value="NGO Forum on Cambodia")
        with col2:
            registration_number = st.text_input("Registration Number", placeholder="e.g., # 123 សជណ")
            registration_date = st.date_input("Registration Date", datetime.today())
            tax_percentage = st.selectbox("Tax Percentage (%)", [0, 5, 10, 15, 20], index=2)
        ...
        with col2:
            party_a_address = st.text_area("Party A Address", placeholder="Full address of Party A", height=80)
        ...
        with col1:
            ...
            party_b_phone = st.text_input("Party B Phone", placeholder="e.g., (+855) 12 345 678")
            party_b_email = st.text_input("Party B Email", placeholder="e.g., consultant@example.com")
            party_b_address = st.text_area("Party B Address", placeholder="Full address of Party B", height=80)
        with col2:
            focal_person_a_name = st.text_input("Focal Person A Name", placeholder="e.g., Mr. Mar Sophal")
            focal_person_a_position = st.text_input("Focal Person A Position", placeholder="e.g., Program Manager")
            focal_person_a_phone = st.text_input("Focal Person A Phone", placeholder="e.g., 012 345 678")
            focal_person_a_email = st.text_input("Focal Person A Email", placeholder="e.g., focal@ngoforum.org.kh")
        ...
        with col3:
            total_fee_usd = st.number_input("Total Fee USD", min_value=0.0, step=0.01, format="%.2f")
        ...
        with col2:
            title = st.text_input("Title", placeholder="e.g., Consultant")
        ...
        if submitted:
            if not all([project_title, contract_number, party_a_name, party_b_full_name_with_title]):
                ...
                if insert_contract(new_data):
                    ...
                    st.markdown('<div class="success">Contract added successfully!</div>', unsafe_allow_html=True)
                    st.rerun()